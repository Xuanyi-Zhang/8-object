...
with graph.as_default():
	...
	weights1 = tf.Variable(tf.truncated_normal([data_size,hidden_nm], stddev=1.0))
	biases1 = tf.Variable(tf.zeros([hidden_nm]))

        weights2 = tf.Variable(tf.truncated_normal([hidden_nm,8], stddev=1.0))
	biases2 = tf.Variable(tf.zeros([8]))
